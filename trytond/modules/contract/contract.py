...
class Contract(ModelWorkflow, ModelSQL, ModelView):
    ...
    list_price = fields.Numeric('List Price', states=STATES,
                               ...
                               help='Fixed-price override. Leave at ' \
                                '0.0000 for no override. Use discount ' \
                                'at 100% to set actual price to 0.0')
    ...
    lines = fields.One2Many('account.invoice.line',
                            'contract', 'Invoice Lines',
                            readonly=True,
                            domain=[('contract', '=', Eval('id'))])
    ...
    def _invoice_init(self, contract, invoice_date):
        ...
        invoice_address = contract.party.address_get(
            contract.party.id, type='invoice')
        config_obj = pool.get('contract.configuration')
        config = config_obj.browse(1)
        ...
        invoice = invoice_obj.create(dict(
            ...
            account=contract.party.account_receivable.id or \
                contract.company.account_receivable.id,
            payment_term=contract.party.payment_term.id or \
                contract.payment_term.id,
    ...
    def _contract_unit_price(self, contract):
        ...
        if contract.list_price:
            ...
            unit_price = contract.product.list_price - \
                    (contract.product.list_price * contract.discount / 100)
        return unit_price
    ...
    def _invoice_append(self, invoice, contract, period):
        ...
        if not unit_price:
            # skip this contract
            log.info("skip contract without unit_price: %s " \
                     "contract.product.list_price: %s " \
                     "contract.list_price: %s contract.discount: %s" % (
                         unit_price,
                         contract.product.list_price,
                         contract.list_price,
                         contract.discount))
            return
        ...
        account = contract.product.get_account(
            [contract.product.id], 'account_revenue_used')
        if account:
            linedata['account'] = account.popitem()[1]

        tax_rule_obj = pool.get('account.tax.rule')
        taxes = contract.product.get_taxes(
            [contract.product.id], 'customer_taxes_used')
        for tax in taxes[contract.product.id]:
            pattern = {}
            if contract.party.customer_tax_rule:
                tax_ids = tax_rule_obj.apply(
                    contract.party.customer_tax_rule, tax, pattern)
                if tax_ids:
    ...
    def create_next_invoice(self, ids, data=None):
        ...
        log.debug("invoice_date: %s period: %s" % (
            invoice_date, period))
    ...
    def _check_contract(self, contract, invoice_date):
        ...
        False is returned if this contract is not
        up for billing at this moment.

        period is defined as (last_date, next_date, quantity)
        where the first two are datetime values indicating
        the billing period this contract is valid for, and
        the last indicates the quantity on the invoice line
        (months, weeks, years)
        ...
        if contract.next_invoice_date and end < contract.next_invoice_date:
            log.info('too early to invoice: %s + %d days < %s' % (
                invoice_date, NOTICE_DAYS, end))
            return False

        last_date = contract.next_invoice_date or \
                contract.start_date or invoice_date
        next_date = last_date
        quant = 0
...
class CreateInvoiceInit(ModelView):
    ...
    invoice_date = fields.Date('Invoice Date',
                               help='Use date for generated invoices.')
